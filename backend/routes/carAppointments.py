...
from controller.carAppointment import (
    get_car_appointment,
    get_car_appointments_by_user_id,
    get_all_car_appointments,
    update_car_appointment,
    delete_car_appointment,
    create_car_appointment,
)
...
def add_car_appointment():
    ...
    appointment = create_car_appointment(
        data["user_id"], CAR_SERVICE_ID, data["date"]
    )
    return (
        jsonify(
            {
...
def modify_car_appointment(appointment_id):
    ...
    return (
        jsonify(
            {"message": f"Couldn't resolve appointment with id : {appointment_id}"}
        ),
        404,
    )
...
def remove_car_appointment(appointment_id):
    if delete_car_appointment(appointment_id):
        return jsonify(
            {"message": f"Car appointment with id : {appointment_id} deleted"}
        )
    return jsonify(
        {"message": f"Couldn't resolve car appointment with id : {appointment_id}"}, 404
    )
